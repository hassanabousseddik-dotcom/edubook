// EduBook Reservations List Component — async Supabase version
...
export function renderReservations(container) {
  ...
  function getStatusBadge(status) {
    const map = {
      approved:  '<span class="badge badge-approved"><span class="badge-dot"></span>Approuvée</span>',
      pending:   '<span class="badge badge-pending"><span class="badge-dot"></span>En attente</span>',
      rejected:  '<span class="badge badge-rejected"><span class="badge-dot"></span>Rejetée</span>',
  ...
  function filterAndRenderTable() {
    ...
    tableBody.innerHTML = filtered.map(res => {
      const eq = equipmentList.find(item => item.id === res.equipmentId) ||
        { name: 'Matériel Supprimé', imageUrl: 'https://images.unsplash.com/photo-1546410531-bb4caa6b424d?w=500&auto=format&fit=crop&q=60', ref: 'N/A' };
      ...
      if ((isPending || isApproved) && !isFinished) {
        actionsHtml += `<button class="btn btn-danger btn-cancel btn-icon-only" data-res-id="${res.id}" title="Annuler">
          <i data-lucide="trash-2" style="width:16px;height:16px;"></i></button>`;
      }
      if (isApproved && !isFinished) {
        if (res.prolongationRequested) {
          actionsHtml += `<span style="font-size:0.75rem;color:var(--warning);font-weight:600;padding:6px 12px;background:var(--warning-glow);border-radius:var(--radius-sm);border:1px solid rgba(245,158,11,0.2);">Prolongation demandée</span>`;
        } else {
          actionsHtml += `<button class="btn btn-secondary btn-prolong" data-res-id="${res.id}">Prolonger d'un jour</button>`;
        }
      }
      ...
      return `
        <tr>
          <td style="font-weight:600;display:flex;align-items:center;gap:12px;">
            <img src="${eq.imageUrl || eq.image_url}" alt="${eq.name}"
              style="width:36px;height:36px;object-fit:cover;border-radius:var(--radius-sm);border:1px solid var(--border-color);" />
            <div>
    ...
    // Prolonger
    tableBody.querySelectorAll('.btn-prolong').forEach(btn => {
      btn.addEventListener('click', async () => {
        const resId = btn.getAttribute('data-res-id');
        btn.disabled = true;
        try {
    ...
    <div class="glass-card" id="table-card-container" style="padding:20px;">
      <div class="table-container">
        <table class="custom-table">
          <thead><tr>
            <th>Matériel</th><th>Période &amp; Horaire</th>
            <th>Activité Pédagogique</th><th>Statut</th><th>Actions</th>
          </tr></thead>
          <tbody id="reservations-table-body"></tbody>
        </table>
      </div>
    </div>
    <div class="glass-card" id="empty-state-container"
      style="display:none;flex-direction:column;align-items:center;justify-content:center;padding:48px;text-align:center;"></div>
  `;
